...
class OBERFLAECHE implements UIOBJEKT
{
	...
	void prozess()
	{
		if(baumodus == true)
		{
			...
			else if ((vorschau.position.x - 30) / 60 >= 25) vorschau.positionSetzen(new VEKTOR(1530, vorschau.position.y));
			if ((vorschau.position.y - 30) / 60 <= 0) vorschau.positionSetzen(new VEKTOR(vorschau.position.x, 90));
			else if ((vorschau.position.y - 30) / 60 >= 17) vorschau.positionSetzen(new VEKTOR(vorschau.position.x, 990));
